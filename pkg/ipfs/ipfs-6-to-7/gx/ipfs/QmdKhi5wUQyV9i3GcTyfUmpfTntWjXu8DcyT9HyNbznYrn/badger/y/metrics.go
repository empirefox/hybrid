...
func init() {
	NumReads = new(expvar.Int)
	NumWrites = new(expvar.Int)
	NumBytesRead = new(expvar.Int)
	NumBytesWritten = new(expvar.Int)
	NumLSMGets = new(expvar.Map).Init()
	NumLSMBloomHits = new(expvar.Map).Init()
	NumGets = new(expvar.Int)
	NumPuts = new(expvar.Int)
	NumBlockedPuts = new(expvar.Int)
	NumMemtableGets = new(expvar.Int)
	LSMSize = new(expvar.Map).Init()
	VlogSize = new(expvar.Map).Init()
	PendingWrites = new(expvar.Map).Init()
}
